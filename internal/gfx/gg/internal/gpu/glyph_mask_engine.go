...
type GlyphMaskEngine struct {
	...
	// device is the wgpu device the page textures were created on. When it
	// changes (Android recreates the device on surface recreation — rotation,
	// background/foreground), the old textures are invalid and must be dropped
	// and re-uploaded to the new device. See SyncAtlasTextures.
	device *wgpu.Device
}
...
func (e *GlyphMaskEngine) SyncAtlasTextures(device *wgpu.Device, queue *wgpu.Queue) error {
	...
	// Device changed: Android recreates the wgpu device whenever the surface is
	// recreated (rotation, background→foreground). The existing page textures
	// belong to the old, now-destroyed device, so sampling them renders garbage
	// — text as solid blocks that persists for the session. Drop them and force
	// a full re-upload of every page to the new device. The atlas glyph cache is
	// CPU-side and stays valid, so no re-rasterization is needed.
	deviceChanged := e.device != nil && e.device != device
	if deviceChanged {
	...
	// On a device change every page needs re-uploading (textures were just
	// dropped); otherwise only the dirty pages do.
	var pages []int
	if deviceChanged {
		for i := 0; i < e.atlas.PageCount(); i++ {
			pages = append(pages, i)
		}
	} else {
		pages = e.atlas.DirtyPages()
	}
	if len(pages) == 0 {
		return nil
	}
