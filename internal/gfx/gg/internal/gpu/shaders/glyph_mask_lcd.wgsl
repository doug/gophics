...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // textureSampleLevel(..., 0.0) — the glyph atlas is single-mip and pre-
    // rasterized at device size; textureSample's derivative-based LOD returns
    // ~1 for minified glyphs on tile-based mobile GPUs (PowerVR), turning text
    // into solid blocks. See glyph_mask.wgsl for the full explanation.
    let cov_r = textureSampleLevel(atlas_texture, atlas_sampler, vec2<f32>(uv_center.x - texel_step, uv_center.y), 0.0).r;
    let cov_g = textureSampleLevel(atlas_texture, atlas_sampler, uv_center, 0.0).r;
    let cov_b = textureSampleLevel(atlas_texture, atlas_sampler, vec2<f32>(uv_center.x + texel_step, uv_center.y), 0.0).r;
