...
func (p *GlyphMaskPipeline) RecordDraws(rp *wgpu.RenderPassEncoder, resources *glyphMaskFrameResources, clipBG *wgpu.BindGroup, depthClipped ...bool) {
	...
	// Pipeline is selected PER DRAW CALL, not once per frame: a single frame may
	// mix grayscale and LCD batches (selectGlyphMaskLCD is per-run), and each
	// batch's bind group is built with — and sized for — its own layout
	// (grayscale = 80-byte uniform, LCD = 96-byte uniform with atlas_size). Binding
	// an 80-byte grayscale group to the 96-byte LCD shader (or vice versa) is a
	// pipeline/bind-group layout + uniform-size mismatch: Metal's validation layer
	// aborts the draw ("argument uniforms[0] ... has space for 80 bytes, but
	// argument has a length(96)"), and release drivers read 16 bytes of garbage.
	// Depth-clip takes priority (frame-level) exactly as before.
	var current *wgpu.RenderPipeline
	for _, dc := range resources.drawCalls {
		if dc.indexCount == 0 {
			continue
		}
		pipeline := p.pipelineWithStencil
		switch {
		case useDepthClip:
			pipeline = p.pipelineWithDepthClip
		case dc.isLCD && p.lcdPipelineWithStencil != nil:
			pipeline = p.lcdPipelineWithStencil
		}
		if pipeline != current {
...
func (p *GlyphMaskPipeline) ensureLCDPipelineWithStencil() error {
	...
	// but with a larger uniform buffer (96 bytes instead of 80). MinBindingSize
	// must be the LCD size — the shader reads atlas_size at offset 80, so an
	// 80-byte binding here would under-provision it.
		...
		Entries: []gputypes.BindGroupLayoutEntry{
			{
				Binding:    0,
				Visibility: gputypes.ShaderStageVertex | gputypes.ShaderStageFragment,
				Buffer:     &gputypes.BufferBindingLayout{Type: gputypes.BufferBindingTypeUniform, MinBindingSize: glyphMaskLCDUniformSize},
			},
			{
				Binding:    1,
				Visibility: gputypes.ShaderStageFragment,
...
type glyphMaskDrawCall struct {
	...
	isLCD       bool // true when this batch must use the LCD pipeline (96-byte uniform)
}
