...
func (i *Instance) EnumerateAdapters(surfaceHint hal.Surface) []hal.ExposedAdapter {
	...
	for _, device := range devices {
		...
		adapter := &Adapter{
			instance: i,
			raw:      device,
			// isDepth24Stencil8PixelFormatSupported is macOS-only; iOS GPUs never
			// support Depth24Stencil8 (they use Depth32Float_Stencil8) and sending
			// the selector to an iOS device crashes. Split by platform.
			formatDepth24Stencil8: deviceSupportsDepth24Stencil8(device),
		}
