...
// Device trait queries (DeviceIsLowPower / DeviceIsHeadless / DeviceIsRemovable /
// DeviceRecommendedMaxWorkingSetSize / deviceSupportsDepth24Stencil8) are split by
// platform in device_traits_darwin.go and device_traits_ios.go: those selectors
// are macOS-only, and on iOS the concrete MTLDevice class (e.g. AGXG14Device)
// implements them as throwing stubs — respondsToSelector: returns YES but the send
// still raises "unrecognized selector" and crashes. A runtime guard cannot detect
// that; only a compile-time platform split is safe.
