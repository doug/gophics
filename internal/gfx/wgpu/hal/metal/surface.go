...
func (s *Surface) Configure(device hal.Device, config *hal.SurfaceConfiguration) error {
	...
	// framebufferOnly must be NO: the present path creates a texture VIEW from
	// the drawable (to bind as the render-pass attachment), and Metal forbids
	// that on a framebufferOnly texture ("texture view creation not allowed from
	// frameBufferOnly textures" — asserts on iOS/macOS). framebufferOnly=YES is
	// only a minor bandwidth optimization for textures used solely as opaque
	// render targets, which is incompatible with how we present. Storage binding
	// already required NO; make it unconditional so drawable views always work.
	msgSendVoid(s.layer, Sel("setFramebufferOnly:"), argBool(false))
