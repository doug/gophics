...
func TestTextWrapsWithinConstraint(t *testing.T) {
	...
	tb.Wrap = false
	tb.MarkDirty()
	single := tb.Layout(layout.Loose(geom.Size{W: 120, H: 1000}))
	if single.H != wide.H {
...
func TestDecorationsPaintLines(t *testing.T) {
	...
	tb.Layout(layout.Loose(geom.Size{W: 80, H: 1000}))
...
func TestTextMemoAcrossConstraintChanges(t *testing.T) {
	...
	s1 := tb.Layout(layout.Loose(geom.Size{W: 120, H: 1000}))
	tb.MarkDirty()                                           // force a real re-layout (not the Base skip-cache)
	s2 := tb.Layout(layout.Loose(geom.Size{W: 120, H: 500})) // memo hit: same width
	if s1 != s2 {
	...
	tb.Text = "tiny"
	tb.MarkDirty()
	s3 := tb.Layout(layout.Loose(geom.Size{W: 120, H: 500}))
...
func TestRichMemoAcrossConstraintChanges(t *testing.T) {
	p := textPainter(t)
	rb := &RichBox{Painter: p, TextSize: 14, Spans: []layout.RichSpan{
		{Text: "the quick brown "}, {Text: "fox jumps over the lazy dog"},
	}}

	s1 := rb.Layout(layout.Loose(geom.Size{W: 120, H: 1000}))
	segs := len(rb.segs)
	rb.MarkDirty()
	s2 := rb.Layout(layout.Loose(geom.Size{W: 120, H: 500})) // memo hit: same width
	if s1 != s2 || len(rb.segs) != segs {
		t.Fatalf("height-only constraint change altered rich layout: %v/%d vs %v/%d", s1, segs, s2, len(rb.segs))
	}

	rb.Spans = []layout.RichSpan{{Text: "tiny"}}
	rb.MarkDirty()
	s3 := rb.Layout(layout.Loose(geom.Size{W: 120, H: 500}))
	if s3.W >= s1.W || s3.H >= s1.H {
