...
func (l *leaf) Layout(cs layout.Constraints) geom.Size {
	return l.Done(cs, cs.Constrain(geom.Size{W: l.w, H: l.h}))
}
func (l *leaf) Paint(c paint.Canvas, at geom.Pt) {}
func (l *leaf) AddHits(p geom.Pt, hits *[]layout.Hit) {
	if l.contains(p) {
		*hits = append(*hits, layout.Hit{Box: l, Pos: p})
	}
}
...
func TestConstraints(t *testing.T) {
	c := layout.Constraints{Min: sz(10, 10), Max: sz(100, 100)}
	...
	if !layout.Unbounded().Constrain(sz(1e9, 1e9)).IsEmpty() == false {
		t.Fatal("unbounded should admit any size")
	}
	if layout.Tight(sz(50, 50)).Constrain(sz(0, 999)) != sz(50, 50) {
...
func TestPadded(t *testing.T) {
	...
	got := p.Layout(layout.Loose(sz(200, 200)))
	if got != sz(70, 40) {
		t.Fatalf("Padded size = %v, want 70x40", got)
	}
	hits := layout.HitTest(p, geom.Pt{X: 15, Y: 15})
	if len(hits) != 2 || hits[0].Box != layout.Box(child) {
		t.Fatalf("hit path = %v, want child first", hits)
	}
	if hits[0].Pos != (geom.Pt{X: 5, Y: 5}) {
		t.Fatalf("child-local pos = %v, want (5,5)", hits[0].Pos)
	}
	if len(layout.HitTest(p, geom.Pt{X: 5, Y: 5})) != 1 {
...
func TestAlignedCenters(t *testing.T) {
	...
	a := Center(child)
	if got := a.Layout(layout.Tight(sz(100, 100))); got != sz(100, 100) {
		t.Fatalf("Aligned should expand, got %v", got)
	}
	hits := layout.HitTest(a, geom.Pt{X: 50, Y: 50})
	if len(hits) != 2 {
		t.Fatalf("center point should hit child, path = %v", hits)
	}
	if len(layout.HitTest(a, geom.Pt{X: 5, Y: 5})) != 1 {
...
func TestSized(t *testing.T) {
	...
	got := s.Layout(layout.Loose(sz(100, 100)))
	if got != sz(30, 5) {
...
func TestFlexRowFixedAndFlexible(t *testing.T) {
	...
	c := &leaf{w: 50, h: 10}
	f := &Flex{Axis: layout.Horizontal, CrossAlign: layout.CrossCenter, Children: []FlexChild{
		{Box: a}, Flexible(1, b), {Box: c},
	}}
	got := f.Layout(layout.Tight(sz(200, 40)))
	if got != sz(200, 40) {
...
func TestFlexWeights(t *testing.T) {
	a, b := &leaf{}, &leaf{}
	f := &Flex{Axis: layout.Horizontal, Children: []FlexChild{Flexible(1, a), Flexible(3, b)}}
	f.Layout(layout.Tight(sz(100, 10)))
...
func TestColumnShrinkWrapsUnboundedMain(t *testing.T) {
	...
	got := f.Layout(layout.Loose(geom.Size{W: 100, H: layout.Inf}))
	if got != sz(20, 70) {
...
func TestFlexMainAlignEnd(t *testing.T) {
	a := &leaf{w: 30, h: 10}
	f := Row(a)
	f.MainAlign = layout.MainEnd
	f.Layout(layout.Tight(sz(100, 10)))
	if f.offsets[0].X != 70 {
		t.Fatalf("layout.MainEnd offset = %v, want 70", f.offsets[0].X)
	}
}
...
func TestHitOrderTopmostFirst(t *testing.T) {
	...
	got := dec.Layout(layout.Loose(sz(50, 50)))
	if got != sz(10, 10) {
		t.Fatalf("decorated wraps child, got %v", got)
	}
	hits := layout.HitTest(dec, geom.Pt{X: 5, Y: 5})
	if len(hits) != 2 || hits[0].Box != layout.Box(inner) || hits[1].Box != layout.Box(dec) {
...
func (l *countingLeaf) Layout(cs layout.Constraints) geom.Size {
...
func TestLayoutSkipCache(t *testing.T) {
	...
	col.Layout(layout.Tight(sz(200, 200)))
