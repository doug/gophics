...
func TestFieldOptionsReachTheSoftKeyboard(t *testing.T) {
	...
	ti := mount(TextField{Autofocus: true, Value: "pw", Obscure: true})
	...
	ti = mount(TextField{Autofocus: true, Value: "a@b", Keyboard: shell.TextInputEmail})
	...
	ti = mount(TextField{Autofocus: true, Value: "x", NoAutocorrect: true})
	if ti.opts.Autocorrect {
		t.Error("NoAutocorrect field still asked for autocorrect")
	}

	ti = mount(TextField{Autofocus: true, Value: "fixed", ReadOnly: true})
	if ti.shown != 0 {
