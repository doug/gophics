...
// Conformance is declared against the *Protocol* names. gomobile emits a
// protocol and a class with the same name for each Go interface, and Swift
// keeps the class under the bare name — so conforming to `MobileShareHost`
// would be inheritance from a class and fails to compile, while reading exactly
// like correct code. Declaring nothing at all, which this did, is worse: the
// methods are all present, the file typechecks on its own, and it cannot be
// registered as any of these hosts.
public final class GophicsPlatform: NSObject,
    MobileShareHostProtocol,
    MobileNotifyHostProtocol,
    ...
    MobileDeviceHostProtocol
{
    ...
    /// Retained: NWPathMonitor stops reporting as soon as it is released.
    fileprivate var pathMonitor: NWPathMonitor?
...
// MARK: - DeviceHost: screen wake, photo library, biometrics

extension GophicsPlatform {
    // MARK: Wake lock
    //
    // The lease counting is on the Go side, so this is only ever told about the
    // transitions — which is what keeps two widgets each holding a lease from
    // having to know about each other.

    public func setKeepAwake(_ on: Bool) {
        UIApplication.shared.isIdleTimerDisabled = on
    }

    // MARK: Photo library

    public func authorizePhotos(_ reqID: Int) {
        // Add-only: an app that saves a picture has no business enumerating the
        // library, and the smaller ask is more likely to be granted.
        PHPhotoLibrary.requestAuthorization(for: .addOnly) { [weak self] status in
            DispatchQueue.main.async {
                let ok = status == .authorized || status == .limited
                self?.bridge.deliverPhotosPermission(reqID, granted: ok)
            }
        }
    }

    public func savePhoto(_ reqID: Int, data: Data?, album: String?) {
        guard let data, !data.isEmpty else {
            bridge.deliverPhotoSaved(reqID, errMsg: "no image data")
            return
        }
        PHPhotoLibrary.shared().performChanges {
            ...
            DispatchQueue.main.async {
                self?.bridge.deliverPhotoSaved(reqID,
                                               errMsg: ok ? "" : (err?.localizedDescription ?? "save failed"))
            }
        }
        // album is ignored: adding to a named album needs the full library
        // permission this deliberately does not ask for.
    }

    // MARK: Biometrics

    public func biometricKind() -> Int {
        let ctx = LAContext()
        var err: NSError?
        guard ctx.canEvaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, error: &err) else {
            return 0 // none enrolled, or locked out after repeated failures
        }
    ...
    public func authenticate(_ reqID: Int, reason: String?, allowFallback: Bool) {
        let ctx = LAContext()
        // With fallback, the device passcode is accepted when biometry fails or
        // is not enrolled — which is what lets someone in on a cold morning
        // when their fingerprint will not read.
        let policy: LAPolicy = allowFallback
            ? .deviceOwnerAuthentication
            : .deviceOwnerAuthenticationWithBiometrics
        let why = (reason?.isEmpty == false) ? reason! : "Authenticate"
        ctx.evaluatePolicy(policy, localizedReason: why) { [weak self] ok, err in
            DispatchQueue.main.async {
                self?.bridge.deliverAuth(reqID, ok: ok,
                                         errMsg: ok ? "" : (err?.localizedDescription ?? "authentication failed"))
            }
        }
    }
}

// MARK: - Pushed state: connectivity and battery
//
// Neither is a host interface, because neither is a question Go can usefully
// ask: the platform already knows and changes the answer on its own schedule.
// Both start *unknown* on the Go side — until observe() reports once,
// ctx.Connectivity() is nil rather than optimistically "online", which is what
// keeps an app from skipping its offline path at launch.

extension GophicsPlatform {
    /// Begins reporting reachability and battery. Call once after registering
    /// the hosts; it is what makes ctx.Connectivity() and ctx.Battery() appear.
    public func observe() {
        let monitor = NWPathMonitor()
        pathMonitor = monitor
        monitor.pathUpdateHandler = { [weak self] path in
            DispatchQueue.main.async { self?.bridge.setOnline(path.status == .satisfied) }
        }
        monitor.start(queue: DispatchQueue(label: "gophics.reachability"))
    ...
    private func reportBattery() {
        let dev = UIDevice.current
        // A simulator reports -1 for an unknown level; sending it as 0 would
        // read as a flat battery rather than as no information.
        let level = dev.batteryLevel
        guard level >= 0 else { return }
        let charging = dev.batteryState == .charging || dev.batteryState == .full
        bridge.setBattery(level, charging: charging)
    }
}
