...
// are separate classes and both drive it. gomobile assumes one anyway: Start
// builds the app once.
...
class MainActivity : Activity() {
    ...
    // Both capture permissions are asked for at runtime, so their results have
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // nothing to mirror.
        monitor = GophicsMonitor(this, bridge).also { bridge.setMonitorHost(it) }
        preview = GophicsPreview(this, bridge).also { bridge.setPreviewHost(it) }
...
class GophicsView(private val activity: Activity) :
    ...
    override fun surfaceChanged(holder: SurfaceHolder, format: Int, w: Int, h: Int) {
        ...
        if (!surfaceSet && nativeWin != 0L) {
            bridge.setSurface(0, nativeWin, w.toLong(), h.toLong(), d.toFloat())
            gpuActive = bridge.gpuActive()
            surfaceSet = true
            Log.i("gophics", if (gpuActive) "GPU present" else "GPU unavailable — CPU blit")
        }
        bridge.resize(w.toLong(), h.toLong(), d.toFloat())
    }
