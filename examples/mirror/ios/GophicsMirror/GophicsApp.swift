// Thin iOS host for the gophics Mirror app (M9 embedding model): the Go side
// (Mirrormobile.xcframework, built by gomobile bind) owns the UI; this host
...
// Held for the process lifetime: the Bridge keeps only a reference the Go side
// cannot retain, so dropping these would deregister the backends.
private var monitor: GophicsMonitor!
private var preview: GophicsPreview!
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication,
        ...
        var err: NSError?
        guard let b = MirrormobileStart(&err) else {
        ...
        // and ctx.CameraPreview() are nil on the Go side and mirror has
        // nothing to mirror. These are the reference hosts from
        // shell/mobile/native, compiled into an app for the first time.
        monitor = GophicsMonitor(bridge: bridge)
        bridge.setMonitorHost(monitor)
        preview = GophicsPreview(bridge: bridge)
        bridge.setPreviewHost(preview)
