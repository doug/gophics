...
func SetAppState(state int) {
	log.Printf("news: app state -> %s", shell.AppState(state))
	// Leaving the foreground is the last moment anything is guaranteed to run
	// on a phone, and the ranking model's writes are deliberately deferred so
	// that scrolling does not write a file per row. Flush them here.
	if shell.AppState(state) != shell.StateActive {
		ui.Flush()
	}
