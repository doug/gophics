// Thin iOS host for the gophics news reader (M9 embedding model): the Go side
// (Newsmobile.xcframework, built by gomobile bind) owns the UI; this host
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication,
        ...
        // Before anything else: where the reader may keep its articles,
        // ranking model, subscriptions and picture cache. Application Support
        // is the right home — it is backed up, and unlike Caches the system
    ...
    /// hard-coded on the Go side.
    private func dataDirectory() -> String {
        let base = FileManager.default.urls(for: .applicationSupportDirectory,
                                            in: .userDomainMask).first!
        let dir = base.appendingPathComponent("news", isDirectory: true)
...
class GophicsView: UIView, UIKeyInput {
    ...
    @objc private func frame(_ link: CADisplayLink) {
        ...
        let loginDomain = NewsmobilePendingLoginDomain()
        if !loginDomain.isEmpty {
            let loginURL = NewsmobilePendingLoginURL()
            NewsmobileClearPendingLogin()
            LoginSheet.present(from: self, domain: loginDomain, url: loginURL)
        }
        syncKeyboard()
    }
    ...
    // consuming the same Newsmobile.A11y* surface. ---
