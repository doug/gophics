# Consolidating the forked substrate into the gossamer repo — plan

**Status: DONE (2026-08-04).** All seven forks are vendored into the gossamer
...
| `gg` | 2D vector renderer (gossamer's `paint` backend) | 128k | 59 | 0 | MIT |
...
So even after consolidation, gossamer still pulls `go-webgpu/{goffi,webgpu}` —
that's the FFI floor the whole thing sits on, and it isn't ours to own.

## Decision: one module, substrate under `internal/`

Collapse all seven into the existing `github.com/doug/gossamer` module as
...
```
github.com/doug/gossamer/                 (one module, one go.mod)
...
```
github.com/doug/gputypes    → github.com/doug/gossamer/internal/gfx/gputypes
github.com/doug/gpucontext  → github.com/doug/gossamer/internal/gfx/gpucontext
github.com/doug/naga        → github.com/doug/gossamer/internal/gfx/naga
github.com/doug/wgpu        → github.com/doug/gossamer/internal/gfx/wgpu
github.com/doug/gogpu       → github.com/doug/gossamer/internal/gfx/gogpu
github.com/doug/gg          → github.com/doug/gossamer/internal/gfx/gg
github.com/doug/audio       → github.com/doug/gossamer/internal/audio
```
...
### Why `internal/` rather than public `gossamer/gpu/…`

`internal/` is the honest signal: **this substrate is gossamer's private plumbing,
not a public API we promise to keep stable.** Go enforces it — nothing outside
`github.com/doug/gossamer/…` can import it, so we keep full freedom to refactor
gg/wgpu without semver obligations to anyone.

The one thing to check first (a bounded audit, already scoped): a few *exported*
gossamer signatures currently name substrate types, e.g.
...
- Grep exported gossamer API for `gg.`/`gogpu.`/`wgpu.`/`gpucontext.`/`gputypes.`.
...
- Fix any gossamer **example** that imports a fork directly (examples are ours; a
...
`gossamer/gpu/gg`) and everything else under `internal/`. Not expected to be
needed.

## Why do this (the payoff)

1. **Atomic cross-stack changes.** The GPU opacity-layer work just done spanned
   `gg` **and** gossamer, and to ship it required editing a separate repo, a
...
2. **Kills the `gomobile`-ignores-`go.work` trap outright.** `gomobile bind` does
   ...
   **no external `doug/*` modules**, so nothing to `replace`; device builds just
   work.
3. **One repo, not seven.** No per-fork tagging, no "which `gg` tag does gossamer
...
- **Repo size.** gossamer's own code is a small fraction of a ~442k-LOC
...
1. **Snapshot import.** For each fork, copy its tree into the target subtree,
   dropping `go.mod`, `go.sum`, `.git`, CI configs, and `examples/` (gossamer has
...
2. **Global import rewrite.** Apply the seven path rewrites above across all `.go`
   files (both the moved substrate and gossamer's own source) with a scripted
...
7. **Green the build across all targets:** `go build ./...`, `go test` (framework
   + gpu-tagged), `GOOS=js/wasm`, `GOOS=android`/`ios` via `gossamer run`, and
...
8. **CI import denylist.** Keep the rule that first-party framework packages
   (`widget`, `paint`, `chart`, …) import only stdlib + gossamer's own layers +
